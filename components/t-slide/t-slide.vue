		...
		<view class="t-slide-box" v-for="(item,index) in items" :key="index">
			<view class="t-slide-area" :style="{height:height+'rpx'}">
				　<view class="t-slide-view">
					　　<view class="t-touch-item" :class=" item.isTouchMove?'touch-move-active':''"
						@touchstart="touchstart($event,item)" @touchmove="touchmove" :data-index="index">
						<view class="t-slide-content" :style="{transform:item.isTouchMove?'translateX(0)':`translateX(${btnWidth*(btnArr.length)}rpx)`,marginLeft:`-${btnWidth*(btnArr.length)}rpx`}" @click="itemClick(item)">
							<!-- <slot v-bind:item="item"> </slot> -->
							<uni-list>
								<uni-list-item :title="item.title" ellipsis=1 :note="item.releasedTime" link></uni-list-item>
							</uni-list>
						</view>
						<view class="t-slide-btn" v-for="(btn,num) in btnArr" :key="num"
							@touchstart="btnClick(btn.events,item)" :style="{width:btnWidth+'rpx',transform:item.isTouchMove?'translateX(0)':`translateX(${btnWidth*(btnArr.length)}rpx)`,
